...
@property(nonatomic, strong)NSMutableArray * interestStatusArray;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _interestStatusArray = [NSMutableArray array];
        for (NSString * title in _titleArray) {
            [_interestStatus setObject:@(0) forKey:title];
            [_interestStatusArray addObject:@(0)];
        }
        if ([InterestingPointDB queryAllData].count != 0) {
            Interest * interest = [InterestingPointDB queryAllData].lastObject;
            [self test:interest];            
        }
        [self initForView];
    }
...
-(void)test:(Interest *)inter{
    NSMutableArray * array = [NSMutableArray array];
    [array addObject:@(inter.mantain)];
    [array addObject:@(inter.bar)];
    [array addObject:@(inter.music)];
    [array addObject:@(inter.stage)];
    [array addObject:@(inter.pic)];
    [array addObject:@(inter.eat)];
    [array addObject:@(inter.bag)];
    [array addObject:@(inter.movie)];
    [array addObject:@(inter.person)];
    [array addObject:@(inter.baskball)];
    [array addObject:@(inter.leaf)];
    [array addObject:@(inter.shirt)];
    _interestStatusArray = array;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell setCellSelectedStatus:_interestStatusArray[indexPath.row]];
    return cell;
}
//选中
